...
**Google Gemini Vision AI × 로컬 픽셀 포렌식을 병렬 실행해 이중으로 판별합니다.**
**빌드 도구 없이 Vanilla JS + Node.js만으로 구현한 풀스택 AI 응용 프로젝트입니다.**

![Node.js](https://img.shields.io/badge/Node.js-18+-339933?logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-4-000000?logo=express&logoColor=white)
![Vanilla JS](https://img.shields.io/badge/Vanilla-JS_ES6+-F7DF1E?logo=javascript&logoColor=black)
![Gemini](https://img.shields.io/badge/Gemini-Vision_API-4285F4?logo=googlegemini&logoColor=white)
![No Build Tool](https://img.shields.io/badge/Build_Tool-None-success)
...
> **"외부 AI 하나만 믿지 않는다."**
> Gemini Vision의 의미적 판단과, 브라우저에서 직접 계산하는 물리적 픽셀 포렌식을
> 병렬로 돌려서 서로를 검증합니다. Gemini가 실패하거나 미설정이어도 로컬 포렌식만으로
> 오프라인 판별이 가능하고, 반대로 Gemini가 "진짜"라고 판단해도 강한 위변조 신호가
> 감지되면 포렌식 점수가 최종 판정을 오버라이드합니다.
...
- [파일 구조](#-파일-구조)
- [전체 분석 플로우](#-전체-분석-플로우)
- [기술 상세 (9개 핵심 구현)](#-기술-상세)
- [정확도 측정 & 자동 최적화](#-정확도-측정--자동-최적화)
- [문제 해결 기록](#-문제-해결-기록)
- [관리자 기능](#-관리자-기능)
- [알려진 한계 및 개선 방향](#-알려진-한계-및-개선-방향)
- [면책 조항](#-면책-조항)
...
| 🎞️ **영상 프레임 분석** | `requestVideoFrameCallback`으로 프레임 단위 딥페이크 탐지 |
| ⚡ **캐시 최적화** | `파일명\|크기\|수정일` 조합의 SHA-256 해시로 동일 파일 재요청 시 Gemini API 호출 0회 |
| 📊 **정확도 평가 & 자동 최적화** | 혼동행렬 기반 정확도·정밀도·재현율·F1 계산 + 랜덤 서치로 가중치·임계값 자동 튜닝 |
| 🛠️ **관리자 대시보드** | 신고 모더레이션, 분석 이력 검색, 공지사항 관리 |
...
이 프로젝트를 이해하는 데 필요한 핵심 개념과, 각 요소가 맡은 역할을 먼저 정리합니다.

| 개념 | 정의 | 이 프로젝트에서의 역할 |
|---|---|---|
| **픽셀 포렌식(Pixel Forensics)** | 이미지의 원본 RGB 픽셀 값을 통계적·수학적으로 분석해 위변조나 인공 생성의 흔적을 찾는 기법 | Gemini 없이도 동작하는 1차 판별 축. CFA·GAN·조명·파일·기하학 5개 모듈로 구성 |
| **CFA (Bayer Pattern)** | 카메라 센서가 RGGB 배열로 빛을 받고 나머지 색을 주변 픽셀로 보간(디모자이킹)하는 촬영 방식 | 실제 사진엔 이 보간 흔적이 남는다는 전제로 AI 이미지와 실사진을 구분하는 신호로 사용 |
| **GAN 체커보드 아티팩트** | GAN의 Transposed Convolution 연산이 남기는 2×2 격자 밝기 패턴 | AI 생성 이미지 특유의 텍스처 이상을 잡아내는 신호 |
| **Web Worker** | 브라우저가 제공하는 API로, 메인 스레드와 완전히 독립된 별도 스레드에서 JS를 실행하게 해주는 기능 | 무거운 픽셀 연산을 백그라운드로 격리해 UI가 멈추지 않게 하는 역할 |
| **UI 블로킹** | JS 실행·렌더링·입력 이벤트 처리가 브라우저 메인 스레드의 같은 콜스택을 공유해서, 무거운 연산 중엔 화면 갱신과 클릭 반응이 전부 멈추는 현상 | 이 프로젝트가 Web Worker를 도입한 직접적 이유 |
| **Chain-of-Thought 프롬프팅** | 결론을 바로 묻지 않고 단계별 추론 과정을 명시적으로 요구하는 프롬프트 설계 기법 | Gemini에게 "조명→해부학→텍스처→딥페이크 신호" 순서로 근거를 만들게 강제해 판단 정확도 향상 |
| **적응형 가중치(Adaptive Weighting)** | 고정 비율이 아니라 입력 특성(해상도)에 따라 두 신호의 기여 비율을 동적으로 조절하는 방식 | 저해상도일수록 Gemini 비중을, 고해상도일수록 로컬 포렌식 비중을 높이는 역할 |
| **다중 모듈 교차검증** | 단일 지표가 아니라 여러 독립 신호가 동시에 임계값을 넘어야 인정하는 판정 방식 | CFA 단독 고점(JPEG 압축 등)으로 인한 오탐을 방지하는 역할 |
...
└── js/
    ...
    ├── admin.js            # 관리자 패널 로직 (정확도 평가 · 가중치 최적화 포함)
...
Gemini 판정이 있을 때는 `aiVerdict`/`deepfakeVerdict` 기반으로 verdict를 결정합니다.
강한 포렌식 신호가 감지되면 포렌식 점수로 오버라이드합니다. 오버라이드 조건은
세 가지 중 하나를 충족해야 합니다:

1. 포렌식 종합 점수 0.45 이상 **+** 독립 모듈 2개 이상 강신호
2. CFA(해상도 보정 후) + 조명 점수가 동시에 0.60 이상
3. 모듈 4개 이상이 동시에 강신호

**모듈별 강신호 임계값:** CFA(해상도 보정값 `cfaScoreEffective`) > **0.55**,
조명·GAN·파일·기하학 각 > **0.45**. CFA는 저해상도에서 `cfaTrust`로 보정된
유효값(`cfaScoreEffective`)을 사용하므로 원본 `cfaScore`와 다를 수 있습니다.
CFA 단독 고점(JPEG 압축·리사이즈 왜곡 등)에 의한 오탐을 방지하기 위해
반드시 2개 이상의 독립 모듈이 동시에 임계값을 초과해야 합니다.
...
`<canvas>`의 `getImageData()`는 브라우저에서 이미지 픽셀 데이터에 접근하는 유일한
방법입니다. 딥페이크 분석은 RGB 픽셀 값을 수학적으로 계산해야 하므로 Canvas가 필수입니다.
...
`setInterval(fn, 33)`은 영상 재생 속도와 무관하게 실행되어 같은 프레임을 중복 처리하거나
건너뛸 수 있습니다. `requestVideoFrameCallback`은 새 프레임이 디코드된 직후 정확히
한 번 호출되어 프레임 낭비가 없습니다.
...
> 프레임 선택은 6프레임 간격 균등 샘플링입니다. 학술적 근거보다는 분석 속도와
> 정확도 사이의 실험적 트레이드오프로 정한 값입니다.

---

### 3. Web Worker + Transferable — UI 블로킹 없는 병렬 연산

**Web Worker란?** 브라우저가 제공하는 API로, 메인 스레드와 완전히 독립된 별도의
스레드에서 JS 코드를 실행하게 해줍니다. JS 언어 자체에는 스레드를 만드는 문법이
없기 때문에, 반드시 브라우저가 제공하는 이 API를 통해서만 별도 스레드를 쓸 수
있습니다.

**UI 블로킹이란?** 브라우저 메인 스레드는 JS 실행·화면 렌더링·클릭이나 스크롤 같은
입력 이벤트 처리를 모두 하나의 콜스택에서 순서대로 처리합니다. 192×192 픽셀에 대해
이중·삼중 for 루프를 도는 픽셀 포렌식 연산을 메인 스레드에서 실행하면, 그 연산이
끝날 때까지 화면 갱신도 클릭 반응도 전부 멈춥니다 — 이게 UI 블로킹입니다. Worker는
자신만의 독립된 콜스택을 가지므로, Worker 안에서 아무리 무거운 연산을 돌려도 메인
스레드의 렌더링·입력 처리에는 영향을 주지 않습니다.

`postMessage()`는 기본적으로 데이터를 Deep Copy합니다. `ImageBitmap`을 Transferable로
지정하면 복사 없이 소유권만 이전되어 메모리 절약과 성능이 동시에 개선됩니다.
...
**픽셀 포렌식이란?** 이미지의 원본 RGB 픽셀 값을 통계적·수학적으로 분석해 위변조나
인공 생성의 흔적을 찾는 기법입니다. 아래 5개 모듈이 각자 다른 물리적 특성을 검사해
0~1 사이의 점수를 산출하고, 이를 가중 합산해 최종 판정에 반영합니다.

#### 4-1. CFA 노이즈 매핑

실제 카메라 센서는 RGGB 배열(Bayer Pattern)로 빛을 기록하고 나머지 색을 주변 픽셀로
보간합니다. 이 과정에서 인접 픽셀 간 통계적 상관관계가 생깁니다. AI 이미지는 픽셀을
처음부터 직접 생성하므로 이 패턴이 없거나 비정상적입니다.
...
GAN이 저해상도 Feature Map을 업샘플링할 때 Transposed Convolution(`ConvTranspose`)을
사용합니다. 이 연산은 인접 픽셀이 커널과 겹치는 횟수가 달라지는 현상을 일으켜, 2×2
격자로 밝기가 교대로 높고 낮아지는 체스판 패턴이 생깁니다.
...
이미지를 4×4 = 16개 구역으로 분할해 각 구역의 평균 밝기와 하이라이트(RGB 모두 240
이상인 픽셀) 비율을 계산합니다. 하이라이트가 가장 많은 구역의 행(row)과 전체 평균
밝기가 가장 높은 구역의 행이 일치하지 않으면 조명 비일관성 신호로 판단합니다.
추가로 수직 밝기 기울기(상단 행 평균 − 하단 행 평균)와 그림자 방향 일관성도 함께
반영합니다.

#### 4-4. 파일 데이터 포렌식 (`analyzeFileDataProxy`)

비네팅, Poisson 노이즈, 코너-센터 분산비를 하나의 함수에서 함께 계산해 `fileScore`로
반환합니다.

**비네팅(Vignetting):** 실제 카메라 렌즈는 광학적 특성으로 이미지 중앙보다 가장자리가
자연스럽게 어둡습니다. AI 생성 이미지는 이 어두워짐이 없거나 비정상적으로 균등합니다.
...
수평 스캔라인 기울기 자기상관(`lineCons`), 에지 방향 엔트로피(`angleEntropy`), 8×8
블록 간 밝기 매끄러움(`blockSmooth`) 세 지표를 계산합니다. AI 생성 이미지는 에지
방향이 지나치게 균등하거나(엔트로피 높음) 블록 경계가 과도하게 매끄러운 경향이 있습니다.

---

### 5. 해상도 기반 적응형 가중치 결합

단순 고정 가중치 대신, 이미지 해상도에 따라 Gemini와 로컬 포렌식의 기여 비율을
동적으로 조절합니다. 저해상도 이미지는 Gemini의 의미 분석에 더 의존하고, 고해상도
이미지는 물리적 흔적이 더 뚜렷하므로 로컬 포렌식 비중을 높입니다.
...
function computeAdaptiveWeights(origWidth, origHeight) {
  ...
  const geminiW = 0.75 - t * 0.20; // 256px 이하: 0.75, 1024px 이상: 0.55
...
SHA-256은 어떤 입력이든 256비트(64자리 16진수) 고정 해시로 변환하는 단방향 함수입니다.
파일 전체 대신 `파일명|크기|수정일` 조합을 해시해 수백 MB 영상도 즉시 식별합니다.
...
번들러(Webpack/Vite)가 없으므로 npm 패키지를 프론트엔드에서 직접 import할 수
없습니다. React `useState`와 유사한 Observer 패턴을 Vanilla JS로 직접 구현했습니다.
...
API 키를 서버에만 보관해 브라우저 소스코드에 노출되지 않도록 합니다. 이미지 데이터는
분석을 위해 서버를 경유해 Gemini로 전달됩니다.
...
프롬프트는 한국어로 작성되어 있으며, 이미지용(`IMAGE_PROMPT`)과 영상 프레임용
(`VIDEO_FRAME_PROMPT`) 두 가지로 분리됩니다.
...
영상 프레임용(`VIDEO_FRAME_PROMPT`)은 얼굴 합성 특화 지침(안면 경계선, 이목구비
왜곡, 노이즈 분포)으로 구성되며 JSON 구조는 동일합니다.
...
관리자 패널(`js/admin.js`)에 판별 로직 자체의 성능을 평가하고 튜닝하는 기능을
구현했습니다.

**혼동행렬 기반 평가**: 테스트 샘플(신호값 + 정답 라벨)에 대해 예측 결과를 채점해
정확도·정밀도·재현율·F1-Score를 계산합니다.

```js
// js/admin.js — evaluateDataset()
...
**랜덤 서치 기반 가중치·임계값 자동 최적화**: 5개 모듈 가중치와 4개 임계값(총 9개
파라미터)의 조합 공간에서 15,000회 무작위 샘플링으로 F1·정확도를 최대화하는 조합을
탐색합니다.

```js
// js/admin.js — runOptimization()
for (let i = 0; i < 15000; i++) {
  const w = { cfa: ..., lighting: ..., gan: ..., file: ..., geo: ...,
              imageFakeThreshold: ..., imageSuspiciousThreshold: ..., ... };
...
> **참고**: 현재 평가에 사용하는 테스트셋은 실제 이미지가 아니라 프로젝트 내부에
> 정의된 24개의 신호값 샘플(`BENCHMARK_DATA`)입니다. 이 평가·최적화 파이프라인
> 자체는 정식으로 구현되어 있지만, 공인된 딥페이크 탐지 벤치마크(FaceForensics++
> 등)로 검증한 결과는 아닙니다 — 자세한 내용은 [알려진 한계](#-알려진-한계-및-개선-방향)
> 참고.
...
**원인**: Gemini가 JSON 앞뒤에 마크다운 코드블록(`` ```json ... ``` ``)을 붙여
반환합니다.
...
**원인**: `postMessage()` 기본 동작은 Deep Copy입니다. 224×224 × 4바이트 × 배치
16개 ≈ 3.06MB를 매 배치마다 복사합니다.
...
**원인**: Transferable로 이전된 `ImageBitmap`은 Worker가 소유합니다. 분석 후
`close()`를 명시적으로 호출하지 않으면 GPU 메모리가 즉시 해제되지 않습니다.
...
**원인**: Same-Origin Policy로 외부 도메인 이미지를 Canvas에 그리면
`getImageData()` 호출 시 `SecurityError`가 발생합니다.
...
| 정확도 평가 & 가중치 최적화 | 혼동행렬 기반 성능 지표 계산, 랜덤 서치로 가중치·임계값 자동 튜닝 |
...
이 프로젝트를 만들며 스스로 발견한 구조적 한계입니다.

1. **verdict 서버 검증 누락**: 분석 엔진(`analyzer.worker.js`)이 생성하는 verdict는
   `FAKE` / `SUSPICIOUS` / `AUTHENTIC` 세 가지뿐이지만, 서버는 클라이언트가 POST로
   전송한 verdict를 검증 없이 저장합니다(`server.js`의 `/api/analyses`). 외부 입력으로
   `DEEPFAKE` 등 임의 값이 DB에 기록될 수 있어, 관리자 통계의 `fakeCount`는
   `FAKE || DEEPFAKE`로 방어적으로 집계됩니다(`server.js:472`). 프로덕션이라면 서버
   측 enum whitelist 검증이 필요합니다.

2. **SHA-256 캐시의 무결성 한계**: 캐시 키가 파일 전체 바이트가 아니라
   `파일명|크기|수정일` 메타데이터를 해싱한 값이라, 파일 *내용*이 바뀌어도 메타데이터가
   우연히 같으면 캐시가 갱신되지 않을 수 있습니다. 완전한 콘텐츠 무결성이 필요하다면
   파일 바이트 스트림 자체를 해싱하는 방식이 더 엄밀합니다.

3. **정확도 검증의 한계**: 위에서 소개한 정확도 평가·자동 최적화 기능은 정식으로
   구현되어 있지만, 평가에 쓰이는 테스트셋이 실제 이미지가 아니라 프로젝트 제작자가
   직접 정의한 24개의 신호값 샘플입니다. 공인된 딥페이크 탐지 벤치마크(FaceForensics++,
   DFDC 등)로 검증한 결과가 아니므로, 실세계 판별 정확도를 보장하는 근거로는 쓸 수
   없습니다. 프로덕션화한다면 공개 데이터셋 기반 실측 검증이 최우선 과제입니다.
